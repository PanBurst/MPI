				...
				while (!con.Finished()) {
					if (sourceWaiting.size() == world_size - 1) {
						...
						if (stat.MPI_TAG == TAG_INIT) {
							...
							MPI_Recv(NULL, 0, MPI_DOUBLE, slave_rank, TAG_INIT, MPI_COMM_WORLD, &stat);
							if (param == NULL) {
							...
							RangeParameters* params = con.GetWork(slave_rank);
							int size = (params->y - params->x) * h;
							double* buff = new double[size];
							MPI_Recv(buff, size, MPI_DOUBLE, slave_rank, TAG_JOB_FINISHED, MPI_COMM_WORLD, &stat);
	...
	while (true) {
		...
		if (pixels == NULL) {
			if (first) {
				...
				if (stat.MPI_TAG == TAG_INIT) {
					...
					pixels = new double[(wEnd-wStart)*h];
					delete[]msg_buffer;
		...
		for (int j = 0; j < (size/h); j++) {
			...
			for (int i = 0; i < h; i++) {
				...
				pixels[j * h + i] = calcMandelFracValue((1.0 * ((double)wStart)) * wStep + offsetX, ((double)i) * yStep + offsetY, maxIters);
			}
		}
		//cout << "JOB_FINISHED SLAVE: " << rank << "   Range:" << wStart << "-" << wEnd << std::endl;
		MPI_Send(pixels, size, MPI_DOUBLE, 0, TAG_JOB_FINISHED, MPI_COMM_WORLD);
